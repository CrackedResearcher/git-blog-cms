      ...
      <div className="max-w-md w-full space-y-8 p-8 rounded-xl shadow-lg backdrop-blur-sm bg-white/10 border border-white/20">
        ...
        <form onSubmit={handleCreateBlog} className="mt-8 space-y-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex items-center justify-center px-4 py-2 border border-transparent text-base font-medium rounded-md text-black bg-white hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 dark:bg-white dark:text-black dark:hover:bg-gray-200"
          >
            {isLoading ? (
              <>
                <svg
        ...
        <p className="text-xs mt-4 text-white/70 text-center">
          Your blog will be created as a new file at{" "}
          <code className="px-1 rounded bg-black/40  text-gray-300">
            blog/{title}.mdx
          </code>{" "}
          and will be accessible on your blog.
        </p>
      </div>
    </div>
  );
